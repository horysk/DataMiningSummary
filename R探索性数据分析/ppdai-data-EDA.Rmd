...
ggplot(aes(x = 年龄 ), data = lc.all) +
  ...
  labs(title = 'Age Histogram', x = '年龄', y = '借贷数量') +
...
`lc.all.samp <- lc.all[sample(1:length(lc.all$ListingId), 1000), ]`

`lc.all.samp <- subset(lc.all.samp, select = c(-借款成功日期, -ListingId) )`
...
`qplot(x = 年龄 , y = 历史正常还款期数, data = lc.all) + geom_point()`

`qplot(x = 年龄 , y = 历史成功借款金额, data = lc.all) + geom_point()`

`qplot(x = 年龄 , y = 历史成功借款次数, data = lc.all) + geom_point()`
